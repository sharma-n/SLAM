...
for i = 1:numParticles
  ...
  for j = 1:m
    ...
    if (particles(i).landmarks(l).observed == false)
      ...
      particles(i).landmarks(l).mu = robot(1:2) + z(j).range*[cos(z(j).bearing + robot(3)); sin(z(j).bearing + robot(3))];
...
##      particles(i).weight = p_0;
      ...
      z_error = [z(j).range - expectedZ(1); normalize_angle(z(j).bearing-expectedZ(2))];
      % TODO: update the mean and covariance of the EKF for this landmark
      particles(i).landmarks(l).mu = particles(i).landmarks(l).mu + K*z_error;
      ...
      particles(i).weight = particles(i).weight * exp(-0.5*z_error'*Q_inv*z_error)/(2*pi*sqrt(det(Q)));
    end
